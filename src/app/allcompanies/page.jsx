...
export default function ComparisonPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${base_url}/getALlcompblogs`);
      setBlogs(response.data);

      // Combine all companies from all blogs
      const allCompanies = response.data.flatMap(blog => blog.company || []);
      const sorted = [...allCompanies].sort((a, b) => b.rating - a.rating);
  ...
  const allCompanies = blogs.flatMap(blog => blog.company || []);
